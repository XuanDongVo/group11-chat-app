    ...
    return (
        <div className="friend-invites-tab">
            {invites.length === 0 ? (
                ...
                invites.map((invite) => (
                    <div className="invite-item" key={invite._docId}>
                        <img className="invite-avatar" src={invite.avatar} alt={invite.name} />
                        <div className="invite-container">
                            <span className="invite-name">{invite.name}</span>
                            <div className="invite-actions">
                                <button className="accept-btn" onClick={() => onAccept(invite._docId!)}>Chấp nhận</button>
                                <button className="reject-btn" onClick={() => onReject(invite._docId!)}>Từ chối</button>
                            </div>
                        </div>
                    </div>
                ))
